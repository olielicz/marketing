...
test("matchKnowledgeBase finds a confident match for a clear question", () => {
  const match = matchKnowledgeBase("my abandoned cart emails aren't sending");
  assert.ok(match);
  assert.equal(match.entry.id, "abandoned-cart-emails-not-sending");
...
test("generateSupportAnswer with useAi=false and confident match never attempts AI", async () => {
  const result = await generateSupportAnswer("how do I set up SMTP for recovery emails", { useAi: false });
...
test("generateSupportAnswer escalates honestly when nothing matches and AI is off", async () => {
  const result = await generateSupportAnswer("can olicommerce book me a flight to mars", { useAi: false });
...
test("generateSupportAnswer with useAi=true but no key falls back to KB honestly", async () => {
  const result = await generateSupportAnswer("SMTP email credentials config help", { useAi: true, openaiApiKey: undefined });
  assert.equal(result.source, "knowledge_base");
...
test("callSupportAI makes a real HTTP call and parses JSON response", async () => {
  ...
  try {
    const result = await callSupportAI("How do I connect Shopify?", { apiKey: "sk-test", apiBaseUrl: `http://127.0.0.1:${port}/v1` });
...
test("generateSupportAnswer with real key + working mock AI reports source=ai honestly", async () => {
  const mockServer = http.createServer((req, res) => {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ choices: [{ message: { content: '{"confident": true, "answer": "AI answer about shipping."}' } }] }));
  });
  await new Promise((resolve) => mockServer.listen(0, "127.0.0.1", resolve));
  const port = mockServer.address().port;
  try {
