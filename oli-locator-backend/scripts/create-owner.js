 ...
 * self-hosted Oli-Locator instance. Refuses to run if an owner already
...
if (existing) {
  console.error(
    ...
    "Refusing to overwrite it. Log in and use the change-password flow instead, or delete data/oli-locator.json\n" +
    "yourself first if you really intend to start over (this also deletes all saved leads, inbox, and call logs)."
  );
  process.exit(1);
}
...
console.log("\n" + "=".repeat(70));
console.log("  OLI-LOCATOR OWNER ACCOUNT CREATED");
console.log("=".repeat(70));
console.log(`  Username:  ${owner.username}`);
console.log(`  Password:  ${password}`);
console.log("=".repeat(70));
console.log("\nSave this password now — it is not stored anywhere in plaintext.");
console.log("\nNext: npm start (server will run on port 4700 by default).\n");
